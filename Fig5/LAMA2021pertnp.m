function M = LAMA2021pertnp(N,R,delta,L,tau,c,t)

%Function to animate the LAM + anticipation (Strömbom and Antia. 2021.
%NEJCS)
...
% end
   ...
   P(1:N,:)=PP(1:N,:); % Update all positions simultaneously
    ...
    %Prevent stuck in mill for strong c
    if c==2 && k>50
        AL50=AL(k-49:k);
        if sum(AL50<0.15)==50 %If last 50 alignment values smaller than 0.15, perturb.
            P(1:N,3)=rand(N,1)*2*pi;
